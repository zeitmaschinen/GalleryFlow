...
app = FastAPI(
    ...
    version="1.3.0",
    description=(
        "API for browsing and managing ComfyUI-generated images "
        "with advanced metadata support"
    )
)
origins = [
    "http://localhost:5173",
...
async def add_folder(
    ...
    if not folder_path.is_absolute():
        raise HTTPException(
            status_code=400,
            detail=(
                "The folder path must be an absolute path (e.g., /Users/name/folder "
                "or C:\\Users\\name\\folder)"
            )
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=(
                f"The folder '{folder_path}' does not "
                f"exist. Please check the path and "
                f"try again."
            )
        )
        ...
        raise HTTPException(
            status_code=403,
            detail=(
                f"Cannot access the folder '{folder_path}'. "
                f"Please check your permissions and "
                f"try again."
            )
        )
    except Exception as e:
    ...
    if not folder_path.is_dir():
        raise HTTPException(
            status_code=400,
            detail=(
                f"The path '{folder_path}' exists but is not a directory. "
    ...
    try:
        created_folder = await crud.create_folder(
            db, schemas.FolderCreate(path=resolved_path_str)
        )
        logger.info(
            f"Folder added to database: {created_folder.path} "
            f"(ID: {created_folder.id})"
        )
        ...
        logger.error(
            f"Failed to add folder: {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=(
                "Failed to add the folder to the library. "
...
async def list_folders(db: AsyncSession = Depends(database.get_db)):
    logger.info(
        "Received request to list folders"
    )
    folders = await crud.get_folders(db)
    return folders
...
async def refresh_folder(
    ...
    logger.info(
        f"Received request to refresh folder ID: {folder_id}"
    )
    folder = await crud.get_folder(db, folder_id)
    if not folder:
    ...
    try:
        ...
        logger.error(
            f"Error during manual scan for folder ID {folder_id}: {e}",
            exc_info=True
        )
        raise HTTPException(
            status_code=500,
...
async def get_image_file(
    ...
    for folder in mapped_folders:
        try:
            ...
            logger.warning(
                f"Error resolving mapped folder path '" 
                f"{folder.path}': {e}"
            )
            continue
    ...
    if not is_allowed:
        logger.warning(
            f"Access denied for image path: "
            f"{resolved_requested_path}. "
            "Not within any "
            "mapped folder."
        )
        raise HTTPException(
            status_code=403,
            detail="Access denied: File path is not within a registered folder.")
    ...
    logger.info(
        f"Serving image: {resolved_requested_path} with media type "
        f"{media_type}, "
        f"cache={cache}"
    )
    response = FileResponse(
        str(resolved_requested_path),
    ...
    if cache:
        ...
        etag = (
            f'"{hash(str(file_stat.st_mtime) + str(file_stat.st_size))}"'
        )
...
async def get_thumbnail(file_path: str = Query(...,
                        ...
                        size: str = Query("medium",
                                          description="Thumbnail size: small (150px) or medium (300px)"),
                        db: AsyncSession = Depends(database.get_db)):
    """Serve optimized thumbnails for faster loading."""
    from .thumbnail_generator import thumbnail_generator

    # Security: Verify the image is in a mapped folder (same as main image
    # endpoint)
    requested_path = Path(file_path)
    if not requested_path.is_absolute():
        raise HTTPException(
    ...
    if not is_allowed:
        logger.warning(
            f"Access denied for thumbnail path: "
            f"{resolved_requested_path}. "
            "Not within any "
            "mapped folder."
        )
    ...
    thumbnail_path = thumbnail_generator.generate_thumbnail(
        str(resolved_requested_path), size)
    ...
    logger.info(
        f"Serving thumbnail: {thumbnail_path}"
    )
...
async def reveal_file(file_path: str = Query(...,
                                             description="Absolute path to the image file to reveal"),
                      db: AsyncSession = Depends(database.get_db)):
    """
    ...
    if not mapped_folders:
        logger.warning(
            "Reveal access denied: No folders "
            "mapped."
        )
        raise HTTPException(status_code=403,
                            detail="Access denied: No folders are mapped.")

    for folder in mapped_folders:
        try:
            mapped_folder_path = Path(folder.path).resolve(strict=True)
            if resolved_requested_path == mapped_folder_path or resolved_requested_path.is_relative_to(
                    mapped_folder_path):
                is_allowed = True
                break
        except FileNotFoundError:
            logger.warning(
                "Mapped folder path not found during validation: '"
                f"{folder.path}'"
            )
            continue
        except Exception as e:
    ...
    if not is_allowed:
        logger.warning(
            f"Reveal access denied for path: {resolved_requested_path}. "
            "Not within any "
            "mapped folder."
        )
        raise HTTPException(
    ...
    if not resolved_requested_path.is_file():
        logger.error(
            f"Cannot reveal file: File not found at resolved path: "
            f"{resolved_requested_path}"
        )
        raise HTTPException(status_code=404,
    ...
    try:
        if system == "Darwin":
            ...
            process = subprocess.run(
                ...
                shell=False)
            logger.info(
                f"Executed 'explorer /select,': {process.args}. "
                f"Return code: "
                f"{process.returncode}"
            )
        elif system == "Linux":
            ...
            logger.warning(
                f"Unsupported operating system for reveal: {system}"
            )
            raise HTTPException(
                status_code=501,
                detail=f"Reveal function not implemented for OS: {system}")

        return {"message": "File location opened in Finder/Explorer."}

    except FileNotFoundError:
        logger.error(
            f"Reveal command not found: '"
            f"{command[0]}' is likely not in the system PATH."
        )
        raise HTTPException(
            status_code=500,
            detail=f"File manager command ('{command[0]}') not found."
        )
    except subprocess.CalledProcessError as e:
        logger.error(
            f"Error executing reveal command: {e}. "
            f"Stderr: {e.stderr}"
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to reveal file: Command execution failed."
        )
    except Exception as e:
        logger.error(
            f"Unexpected error revealing file '"
            f"{resolved_path_str}': {e}"
        )
        raise HTTPException(
            status_code=500,
            detail="An unexpected error occurred during the reveal process."
        )
...
async def root():
    return {
        "message": "GalleryFlow Backend is running!"
    }
